...
def get_subject_id(subject):
        ...
        return (execute_query("SELECT subject_id from Subject where subject_name = %(subject)s", params)[0]['subject_id'])
...
def update_user_test_score(username,subject,score):
        ...
        execute_update("DELETE FROM Exam WHERE user_id = %(user_id)s AND subject_id = %(subject_id)s", params)
        # Insert new test data

        params = {'user_id': user_id,'subject_id': subject_id, 'score': score}
        execute_update("INSERT INTO Exam (user_id, subject_id, grade) VALUES (%(user_id)s, %(subject_id)s, %(score)s)", params)
...
def get_flag_question():
    ...
    wrong_answers_data = execute_query("""
                                            SELECT country_name
                                            FROM Country
...
def get_biggest_cities_question():
    ...
    correct_answer_data = (execute_query("""
