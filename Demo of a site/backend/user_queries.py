...
def delete_user(username):
    try:
        ...
        params = {'username': username}
        execute_update("DELETE FROM User WHERE username = %(username)s", params)
    except DatabaseQueryError as err:
        raise DatabaseError("An error occurred while deleting user.") from err

def count_users():
    try:
        res = execute_query("SELECT COUNT(*) AS user_count FROM User")
...
def get_average_score(username):
    try:
        params = {'username': username}
        res = execute_query("""
